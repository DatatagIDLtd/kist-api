...
namespace kist_api.Services
{
    public class ScanService : IScanService
    {
        ...
        public async Task<List<GeoLocationEvent>> GetScansByLocation(GetScanByLocationRequest req)
        {
   

            var res = new List<GeoLocationEvent>();
            ...
            using (var response = await _client.PostAsync(url, content))
            {
                ...
                res = JsonConvert.DeserializeObject<List<GeoLocationEvent>>(JObject.Parse(apiResponse).GetValue("value").ToString());
